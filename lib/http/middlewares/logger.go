...
func mayDumpBody(ctp string) bool {
	allowedTypes := []string{
		"application/json",
...
func (mw *loggerMW) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// ignore body if is a file
	reqDump, err := httputil.DumpRequest(r, mayDumpBody(r.Header.Get("Content-Type")))
	if err != nil {
	...
	if mayDumpBody(w.Header().Get("Content-Type")) {
